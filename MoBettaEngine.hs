...
boolCalc (Reln cOp expr1 expr2) = do
  c1 <- intCalc expr1
  c2 <- intCalc expr2
  return ((fromMaybe(error "This should not happen") (lookup cOp relnOps))c1 c2)

boolCalc (BBin op expr1 expr2) = do
  c1 <- boolCalc expr1
  c2 <- boolCalc expr2
  return ((fromMaybe(error "This shouldn't happen")(lookup op bBinOps)) c1 c2)
boolCalc (BUn op expr) = do
  c <- boolCalc expr
  return ((fromMaybe(error "This shouldn't happen")(lookup op bUnOps)) c)
...
intCalc (AUn op expr) = do
  c <- intCalc expr
  return ((fromMaybe(error "This should never happen")(lookup op aUnOps)) c)
